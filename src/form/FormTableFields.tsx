...
export const TrSelect :FC<SelectProps> = ({ name, label, value, options, help, onChange }) => {
    const attrs = useMemo(() => {
      let a :any= {
        name,
        label,
...
export const TrInput :FC<InputProps> = ({ name, label, value, type, onChange }) => {
    return (
...
export const TrSubmitButton: FC<SubmitProps>= ({ name, variant, value, disabled, onClick }) => (
    <p className="submit">
        <input
        type="submit"
        name={name}
